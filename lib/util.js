...
const csvToObject = string => {
   ...
   * in string. If there are spaces, yargs.js/format.js/I might have fucked
...
/**
 * Takes an array of comma separated key:value pairs and turns it into an
 * array of JSON objects.
 * @param {Array.<string>} array The array of comma separated key:value pairs
 * @return {Array.<Object>}
 */
const arrayCsvToObject = array => {
  return array.map(csvToObject)
}

/**
 * This function is passed into yargs as a value for coerce in order
 * to get the contents of a file from the filepath.
 * @param {string} filename The filename, including the path
 * @return {string}
 */
const fileReadCoercionFunction = filename => {
  // eslint-disable-next-line no-sync
  return fs.readFileSync(filename, 'utf-8')
}

/**
 * Given a yargs.argv object, this function filters out the specific keys
 * from the argv object into a new object.
 * @param {Object} object The yargs argument object
 * @param {Array.<string>} keys The list of keys to filter out
 * @return {Object}
 */
