 ...
 * @return {string}
 */
const colorCertificateStatus = status => {
  status = String(status)
  switch (status) {
  case 'pending':
    return status.yellow
 ...
 * @param {?string=} message An optional message to display
 */
exports.displayCertificate = (certificate, message) => {
  if (json()) {
    console.log(certificate)
  } else {
    const table = new Table()
    table.push(...colorTable([
      ['ID', colorID(certificate.id)],
      ['Name', colorName(certificate.name)],
      ['Fingerprint', certificate.sha1_fingerprint],
      ['Type', certificate.type],
      ['Status', colorCertificateStatus(certificate.state)],
      ['Created On', formatDate(certificate.created_at)],
      ['Expires On', formatDate(certificate.not_after)]
    ]))
    if (typeof message === 'string') {
      console.log(message.red)
    }
  ...
  if (json()) {
    ...
    if (certificates.length > 0) {
      table.push(...certificates.map(certificate => {
        return [
          colorID(certificate.id),
          colorName(certificate.name),
          formatDate(certificate.not_after),
          colorCertificateStatus(certificate.state)
        ]
      }))
    } else {
      pushNone(table)
...
exports.displayDroplet = (droplet, message) => {
  if (json()) {
    ...
    table.push(...colorTable([
      ...
      ['Created On', formatDate(droplet.created_at)]
    ]))
    table.push([{
      colSpan: 2,
...
exports.displayVolume = (volume, message) => {
  if (json()) {
    ...
    table.push(...colorTable([
      ...
      ['Created On', formatDate(volume.created_at)]
    ]))
    if (typeof message === 'string') {
      console.log(message.red)
    }
