 ...
 * @param {?boolean=} distribution Whether or not to show the distribution
 */
exports.displayImages = (images, sortName, distribution) => {
  if (json()) {
    console.log(images);
  } else {
    var table = new Table({
  ...
  if (json()) {
    ...
    if (kernels.length > 0) {
      kernels.map((kernel) => {
        table.push([{
          colSpan: 2,
          content: 'ID: '.red + colorID(kernel.id)
        }]);
        table.push.apply(table, colorTable([
          ['Name', colorName(kernel.name)],
