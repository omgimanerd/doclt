...
exports.displayFirewall = firewall => {
  if (json()) {
    ...
    table.push(...format.colorTable([
      ...
      ...format.formatInboundFirewallRules(firewall.inbound_rules),
      ...format.formatOutboundFirewallRules(firewall.outbound_rules),
      ['Droplets Assigned',
        format.colorID(format.defaultJoin(firewall.droplet_ids))],
  ...
  if (json()) {
    ...
    if (firewalls.length > 0) {
      table.push(...firewalls.map(firewall => {
        return [
          format.colorID(firewall.id),
          format.colorName(firewall.name),
...
exports.displayLoadBalancer = loadBalancer => {
  if (json()) {
    ...
    const table = new Table()
    table.push(...format.colorTable([
      ['ID', format.colorID(loadBalancer.id)],
      ['Name', format.colorName(loadBalancer.name)],
      ['IP', loadBalancer.ip],
      ['Algorithm', loadBalancer.algorithm],
      ['Status', format.colorLoadBalancerStatus(loadBalancer.status)],
      ...
      table.push(...[
        ...
        ...rules.map(rule => {
          return [
            ...
            format.colorID(rule.certificate_id),
